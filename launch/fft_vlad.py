...
def run(args):
    ...
    if os.path.exists(os.path.join(cfg['out_dir'], f"{loc_exp}_{ref_exp}_v4_recalls.csv")):
        return
    ...
    plt.savefig(os.path.join(cfg['out_dir'], 
        f"{loc_exp}_{ref_exp}_v4_distances.png"))
    ...
    csv_file = os.path.join(cfg['out_dir'], f"{loc_exp}_{ref_exp}_v4_recalls.csv")
